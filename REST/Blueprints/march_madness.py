...
march_madness = Blueprint('march_madness', __name__)


@march_madness.route('/', methods=["POST", "GET"])
def index():
    if request.method == "GET":
        ...
        return redirect(url_for("march_madness.bracket", bias=bias, winner=winner, preferences=preferences))

@march_madness.route('/bracket/<bias>/<winner>/<preferences>')
def bracket(bias, winner, preferences):
    ...
    bracket1.generate_rounds(round_odds, int(bias), pref, winner)
...
# 5) added new bracket button to bracket.html
# 6) add preferences into bracket generation algorithm